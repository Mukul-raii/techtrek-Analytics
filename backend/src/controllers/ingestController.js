...
exports.manualIngest = catchAsync(async (req, res) => {
  ...
  if (!source) {
    throw new ApiError(400, 'Source is required. Must be github, hackernews, or all');
  }
  
  const validSources = ['github', 'hackernews', 'all'];
  if (!validSources.includes(source.toLowerCase())) {
    throw new ApiError(400, 'Invalid source. Must be github, hackernews, or all');
  }
  
  logger.info(`Manual ingestion triggered for source: ${source}`);
  
  // Start ingestion asynchronously
  ingestService.ingestData(source.toLowerCase())
    .then((result) => {
      logger.info(`Ingestion completed for ${source}: ${JSON.stringify(result)}`);
    })
    .catch((error) => {
